...
app.add_middleware(
    CORSMiddleware,
    allow_origin_regex=".*", # Safely allow all origins with credentials
...
async def upload_paper(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        for file in files:
            ...
            file_path = os.path.join(upload_dir, f"{local_id}.pdf")
        ...
        return {
            "success": True, 
            "message": f"Successfully synthesized {len(results)} local units into the intelligence matrix.", 
            "results": results
        }
    except Exception as e:
