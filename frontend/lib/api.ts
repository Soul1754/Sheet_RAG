...
export const api = {
    ...
    getRecommendationsByQuery: async (query: string) => {
        return axios.get<{ recommendations: RecommendedPaper[] }>(`${API_URL}/recommendations/query/${query}`);
    },
    getRecommendationsByPaper: async (paperId: string) => {
        return axios.get<{ recommendations: RecommendedPaper[] }>(`${API_URL}/recommendations/paper/${paperId}`);
    },
    // Papers Library
    getPapers: async () => {
